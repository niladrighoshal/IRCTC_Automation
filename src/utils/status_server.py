...
# This dictionary will store the status of each bot. It's shared in memory.
# A lock is used to ensure thread-safe access.
STATUS_DATA = {}
...
class StatusServerHandler(http.server.SimpleHTTPRequestHandler):
    """
    Custom HTTP request handler for the status server.
    """
    def do_GET(self):
        if self.path == '/status':
